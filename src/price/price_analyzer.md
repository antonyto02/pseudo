...
* Import handle_price_down()    from   price/price_down.rs
    ...
    function analyzePriceUpdate(new_best_bid_ticks)


        last_bid_price = getLastBidPrice()

        Si last_bid_price != null 
                Si new_best_bid_ticks > last_bid_price, entonces handle_price_up()
                Si new_best_bid_ticks < last_bid_price, entonces handle_price_down(new_best_bid_ticks)

        updateLastBidPrice(new_best_bid_ticks)
